...
namespace ProGrafica
{
    public static class Generador3D
    {
        ...
        public static Parte CrearCubo(Vertice centro, float tamaño = 1.0f, Vertice color = null)
        {
            ...
            // Vértices locales (centro en 0,0,0)
        ...
        public static Parte CrearEsfera(Vertice centro, float radio = 1.0f, int segmentos = 16, Vertice color = null)
        {
            ...
            // ✅ CENTRO LOCAL para todos los lados (0,0,0)
            var centroLocal = new Vertice(0, 0, 0);

            // Precalcular vértices para mejor rendimiento
            ...
            for (int i = 0; i <= segmentos; i++)
            {
                ...
                for (int j = 0; j <= segmentos; j++)
                {
                    ...
                    // Calcular vértice en coordenadas locales
            ...
            // Crear lados (cuadriláteros) de la esfera
            for (int i = 0; i < segmentos; i++)
            {
                for (int j = 0; j < segmentos; j++)
                {
                    // ✅ Obtener 4 vértices para formar un lado
                    var v1 = vertices[i, j];
                    var v2 = vertices[i + 1, j];
                    var v3 = vertices[i + 1, j + 1];
                    var v4 = vertices[i, j + 1];

                    // ✅ Crear lado con centro local
        ...
        public static Parte CrearCilindro(Vertice centro, float radio, float altura, int segmentos = 20, Vertice color = null)
        {
            ...
            // Centro local único (0,0,0) para todos los lados
            var centroLocal = new Vertice(0, 0, 0);

            // Precalcular ángulos y vértices base
            ...
            for (int i = 0; i < segmentos; i++)
            {
                ...
                var v1 = verticesBaseInferior[i];    // Punto inferior actual
                var v2 = verticesBaseInferior[next]; // Punto inferior siguiente
                var v3 = verticesBaseSuperior[next]; // Punto superior siguiente  
                var v4 = verticesBaseSuperior[i];    // Punto superior actual
        ...
        public static Objeto CrearMesa(Vertice centro, float ancho = 20f, float alto = 10.0f, float profundo = 10f, Vertice color = null)
        {
            ...
            // Patas - posición relativa al centro de la mesa
            ...
            // Centros de las patas relativos al centro de la mesa
